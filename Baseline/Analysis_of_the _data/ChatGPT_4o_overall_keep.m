...
%%%%%%%%%%%%%%%%%%
% Where we get Figure 2b (Amount kept by AI in the keep framing (out of
% $100))
%%%%%%%%%%%%%%%%%%
...
for i = 1:14
    filename = ['output_chatgpt_session', num2str(i), '.xlsx'];
...
means = [mean(female_keep), mean(male_keep), mean(unspecified_keep)];

%95% CI
alpha = 0.05; 
n_f = length(female_keep);
n_m = length(male_keep);
n_u = length(unspecified_keep);

se_f = std(female_keep)/sqrt(n_f);
se_m = std(male_keep)/sqrt(n_m);
se_u = std(unspecified_keep)/sqrt(n_u);
...
y_max_12 = max(means([1 2]) + ci([1 2]));
y = y_max_12 + gap;

plot([x1 x1 x2 x2], [y y+h y+h y], 'k', 'LineWidth', 1.2)
text(mean([x1 x2]), y+h+1, '***', ...
...
saveas(gcf, 'ChatGPT-4o-overall-keep.png');